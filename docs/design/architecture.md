...
各パッケージは `__init__.py` の `__all__` で公開APIを定義する。
...
```
ビジネスロジック層（<feature>/）
    ↓ Protocol（Port）経由で依存  ← <protocol>/ を import
<protocol>/（Port定義）
    ↑ 実装（明示的継承）
...
テスト時にモック/スタブに差し替えることでテスタビリティを確保する。実際のネットワーク通信やディスクアクセスなしにユニットテストを実行できる。
...
本プロジェクトでは、Adapter（本番実装）と Fake（テスト実装）で Protocol への準拠方法を使い分ける。
...
CLI層は以下のパターンで構成されます。詳細は [CLI モジュール基本設計](../specs/cli/design.md) を参照してください。
...
環境設定は3つのコンポーネントで構成される。詳細は [Config パッケージ基本設計](../specs/config/design.md) を参照してください。
...
設定値の優先度は CLI引数 > 環境変数 > デフォルト値で解決される。
...
- [仕様ドキュメント設計](specs.md): 仕様ドキュメントの構成と運用ルール
