...
- 原則作成しない（pytestはパッケージ化不要、詳細は [packaging.md](packaging.md) を参照）
- `__init__.py` があると import 経路の混乱や名前空間衝突の原因になる
...
**❌ テスト `__init__.py` を作成している**:

理由: pytestはパッケージ化を要求しない。
ファイルが存在するだけで import 経路の混乱や名前空間衝突の原因になる。
...
- **`__init__.py`**: プロダクションでは必須（Docsセクションを常に記述）、テストでは作成しない
