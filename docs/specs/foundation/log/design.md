...
引数・戻り値が大量データを含む場合、ログの肥大化を防ぐため要約形式で出力する。対象はリスト・タプル・辞書・文字列で、要素数または文字数が閾値を超えた場合に先頭要素と総数を表示する。閾値の具体的な値は「値フォーマットの閾値定数化」セクションを参照。
...
`log` デコレータはクラスメソッドと通常関数を実行時に判定する。判定は「第1引数がそのクラスに対象の関数名と同名の属性を持つか」で行われる。
