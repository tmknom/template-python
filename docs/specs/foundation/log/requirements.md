...
- 例外が発生した場合はログ出力を行わず、例外をそのまま呼び出し元に伝播できる
...
閾値の具体的な値は[設計書の「値フォーマットの閾値定数化」セクション](./design.md)で定義する。
