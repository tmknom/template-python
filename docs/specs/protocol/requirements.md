...
ファイルシステム操作の読み取りと書き込みを抽象化したProtocolを提供する。いずれも `typing.Protocol` を使った構造的部分型として定義されており、明示継承なしでも準拠できる。
...
- Python標準ライブラリ `typing.Protocol` のみを使用する
- `pathlib.Path` を使用してファイルパスを表現する
